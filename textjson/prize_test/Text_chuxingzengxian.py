...
author: daizhiwei
Project: 出行赠险-万元出行保障限时送
interlinkage: https://t.shijiala.com/3fiv
'''
import unittest
import warnings
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_tuniu(self):
        ...
        self.driver.find_element_by_id('com.tencent.mm:id/iy0').send_keys(
            'https://t.shijiala.com/3fiv')
        time.sleep(1)
        self.driver.find_element_by_id ('com.tencent.mm:id/anv').click ( )  # 点击发送按钮
        time.sleep(1)
        TouchAction (self.driver).tap (x=465.7, y=887.2).release ( ).perform ( )  # 点击链接
        time.sleep(10)
        # 页面返回 获取title内容
        # a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
        #     '//android.widget.TextView[@text="20万保障免费领"]'))
        # self.assertEqual(a.text, "20万保障免费领")  # 跳转奖品页面之后，对比title，看是否领取成功
        # print("页面正常")
        # time.sleep(5)
        # 屏幕截图，保存指定文件夹时，需要确保对应的文件夹已创建，否则无法保存成功
        self.driver.get_screenshot_as_file(u'/Users/wangmingxiao/Desktop/王明月/text-python/出行赠险-万元出行保障限时送.png')
        print("测试通过")
        time.sleep(10)
