...
author: daizhiwei
Project: 推啊2-翻翻有惊喜-好礼等你拿
interlinkage: https://engine.peonyta.com/index/activity?appKey=2dHtSgioziAHZkiHzzy4x4Ux98Hy&adslotId=352962
'''
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_tuia(self):
        time.sleep(2)
        self.driver.find_element_by_id('com.tencent.mm:id/iy0').click()
        time.sleep(1)
        self.driver.find_element_by_id('com.tencent.mm:id/iy0').send_keys(
            'https://engine.peonyta.com/index/activity?appKey=2dHtSgioziAHZkiHzzy4x4Ux98Hy&adslotId=352962')
        time.sleep(1)
        self.driver.find_element_by_id ('com.tencent.mm:id/anv').click ( )  # 点击发送按钮
        time.sleep(1)
        TouchAction(self.driver).tap(x=309, y=831).release().perform()  # 点击链接
        time.sleep(20)

        # # 页面返回 获取title内容
        # a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
        #     '//android.widget.TextView[@text="翻翻有惊喜"]'))
        # self.assertEqual(a.text, "翻翻有惊喜")  # 跳转奖品页面之后，对比title，看是否领取成功
        self.driver.get_screenshot_as_file(u'/Users/wangmingxiao/Desktop/王明月/text-python/推啊2-翻翻有惊喜-好礼等你拿.png')
        print("页面正常")
        time.sleep(10)
