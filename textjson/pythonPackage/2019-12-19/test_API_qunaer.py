...
class TestSuite_API(unittest.TestCase):
    ...
    # 定义测试去哪儿链接方法
    def test_caseN_link_qunaer(self):
        ...
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
            'https://api.qcoder.com.cn/webService/shopGather/qunaer/index.html')
        self.driver.find_element_by_id ('com.tencent.mm:id/aql').click ( )
        time.sleep (0.5)
        TouchAction (self.driver).tap (x=417, y=790).release ( ).perform ( )
        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="去哪儿大玩卡"]'))
        # self.assertEqual (a.text.encode('UTF-8'), "去哪儿大玩卡", '链接异常')
        self.assertEqual (a.text, "去哪儿大玩卡")
        # time.sleep(0.5)
