...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_yilehudongwangzhezhijian(self,number=1):
        # 点击微信输入框
        # 舞动江湖，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'http://api.qcoder.com.cn/connector/theconnect/targetCon.shtml?type=3&id=2c9080e26cc30cf2016cd291c1e00000')
        ...
        # 点击领取按钮
        time.sleep(4)
        TouchAction(self.driver).tap(x=504.5,y=1607.3).release().perform()
        # 点击。。。
        time.sleep(4)
        ...
        # 点击下载游戏
        time.sleep(6)
        TouchAction(self.driver).tap(x=542.5, y=1683.3).release().perform()
        # 应用宝点击下载
        time.sleep(6)
        TouchAction(self.driver).tap(x=495.5, y=1963).release().perform()
        ...
        # 点击打开应用
        time.sleep(10)
        TouchAction(self.driver).tap(x=531.5,y=1950)
        time.sleep(6)
        # 进入游戏出现的弹窗
