...
class TestSuite_API(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.caps["platformVersion"] = "10"    # 手机的设备的版本
        self.caps["deviceName"] = "HUAWEI Mate 10"   # 手机的名称
        # self.caps["udid"] = "85f8a8a0"             # 需要测试手机的ID（通过adb devices可获取）
        self.caps["udid"] = "192.168.1.21:9966"
    ...
    def test_case_link_shuidibao(self,number=1):
        # 点击微信输入框
        # 水滴保--赠险 ，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
            'https://api.qcoder.com.cn/webService/shopGather/mb3tcsdb/xpp.html')
        ...
        # 点击领取奖励按钮
        time.sleep(12)
        TouchAction(self.driver).tap(x=816.2, y=1878.1).release().perform()
        time.sleep(6)
        # 页面返回 获取title内容
        # 出现地理位置授权的弹窗
        ...
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="同程艺龙"]'))
        self.assertEqual(a.text, "同程艺龙")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
