...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_pinduoduo(self,number=1):
        # 点击微信输入框
        #  拼多多-天天领红包1--cpc，领优惠券链接
        time.sleep(1)
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
            'https://p.pinduoduo.com/HrXr8r1V')

        self.driver.find_element_by_id ('com.tencent.mm:id/aql').click ( )  # 点击发送按钮
        time.sleep(0.5)
        TouchAction (self.driver).tap (x=498.5, y=1223).release ( ).perform ( )  # 点击链接
        # 点击领取奖励按钮

        # TouchAction(self.driver).tap(x=529.5, y=857.5).release().perform()
