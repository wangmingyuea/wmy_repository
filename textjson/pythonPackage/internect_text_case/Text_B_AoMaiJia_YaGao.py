...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_metuan(self,number=1):
        # 点击微信输入框
        # 奥买家-惠宝牙膏套装24.8元包邮，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://d1.wenjuan.link/s?z=wenjuan&c=277&op=1&_sp=3')
        ...
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="WhiteGlo澳洲原装进口牙膏套装24.8包邮 - 奥买家"]'))
        self.assertEqual(a.text, "WhiteGlo澳洲原装进口牙膏套装24.8包邮 - 奥买家")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
