...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_jingshuiqi(self,number=1):
        # 点击微信输入框
        # 净享优物-康佳净水器，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://shop42459384.youzan.com/v2/ump/promocard/fetch?alias=jdj8qe7r')
        time.sleep(0.5)
        ...
        # 点击领取按钮
        time.sleep(3)
        TouchAction(self.driver).tap(x=521.5,y=1373.4).release().perform()
        # 点击进店逛逛按钮
        time.sleep(2)
        TouchAction(self.driver).tap(x=537.5,y=1354.4).release().perform()

        # 页面返回 获取title内容
        time.sleep(3)
        a = WebDriverWait(self.driver, 7).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="好物诚品"]'))
        self.assertEqual(a.text, "好物诚品")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
