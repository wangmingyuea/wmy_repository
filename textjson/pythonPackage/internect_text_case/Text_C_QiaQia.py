...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_qiaqia(self,number=1):
        # 点击微信输入框
        # 洽洽奖券，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://qq-ticket.99hr.com/coupon.html')
        ...
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="幸运用户"]'))
        self.assertEqual(a.text, "幸运用户")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
