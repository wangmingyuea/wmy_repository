...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_tuniu(self,number=1):
        # 点击微信输入框
        # 途牛200，领优惠券链接
        ...
        # 点击领取奖励按钮
        time.sleep(2)
        TouchAction(self.driver).tap(x=520.5, y=1147.5).release().perform()
        time.sleep(3)
        # 页面返回 获取title内容
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="途牛200元出行礼包"]'))
        self.assertEqual(a.text, "途牛200元出行礼包")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
