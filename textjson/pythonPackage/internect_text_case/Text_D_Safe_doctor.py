...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_safedoctor(self,number=1):
      # 点开微信输入框，输入平安好大夫链接
     self.driver.find_element_by_id('com.tencent.mm:id/aqe').send_keys(
          'http://api.qcoder.com.cn/webService/secureGather/heiniu/index.html')
     ...
     # # 点击"拼多多"
     # time.sleep(2)
     # TouchAction(self.driver).tap(x=789.2, y=2267).release().perform()

     # 页面返回 获取title内容
     a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="免费领取出行保障"]'))
     self.assertEqual(a.text, "免费领取出行保障")  # 跳转奖品页面之后，对比title，看是否领取成功
     print("领取成功")
