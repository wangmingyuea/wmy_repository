...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_shuidibao_xindan(self,number=1):
        # 点击微信输入框
        # 水滴保-免费领取 500元健康基金-新旦，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://www.sdbao.com/lab/u/00d9f1154d2379e8?subchannel=zjzx102')
        time.sleep(0.5)
        self.driver.find_element_by_id ('com.tencent.mm:id/aql').click ( )  # 点击发送按钮
        time.sleep(0.5)
        TouchAction (self.driver).tap (x=468.6, y=1130).release ( ).perform ( )  # 点击链接
