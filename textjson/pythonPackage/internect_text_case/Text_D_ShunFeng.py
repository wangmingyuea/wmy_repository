...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_shunfeng(self,number=1):
        # 点击微信输入框
        # 顺丰快递抵扣券2元起--免费，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://nwif-sf-self-creation.weixinjia.net/mobile/index?channel=2&aid=nwIfekVsrUvZwbiAqv3VvfigoaIQJFzs')
        ...
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="赶紧领取顺丰福利"]'))
        self.assertEqual(a.text, "赶紧领取顺丰福利")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
