...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_fenqile(self,number=1):
        # 点击微信输入框
        # 分期乐，领优惠券链接
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
         'https://action.fenqile.com/VlVdTUhcV1RFSVZQWURA/index.html?launch_id=19643')
        time.sleep(0.5)
        ...
        a = WebDriverWait(self.driver, 8).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="分期乐-最快1分钟放款"]'))
        self.assertEqual(a.text, "分期乐-最快1分钟放款")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("领取成功")
