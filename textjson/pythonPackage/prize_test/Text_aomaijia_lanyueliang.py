...
author: daizhiwei
Project: 奥买家-蓝月亮洗衣液特价包邮-直客（无到期时间）
interlinkage: https://m.aomygod.com/act-richang-waituilanyueliang.html?tk=ceb0WhQhqdZ
'''
import unittest
import warnings
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_lin_aomaijia_lanyueliang(self):
        time.sleep(2)
        self.driver.find_element_by_id('com.tencent.mm:id/al_').send_keys(
        'https://m.aomygod.com/act-richang-waituilanyueliang.html?tk=ceb0WhQhqdZ')
        time.sleep(1)
        ...
        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="蓝月亮洗衣液券后29.9元包邮 - 奥买家"]'))
        self.assertEqual(a.text, "蓝月亮洗衣液券后29.9元包邮 - 奥买家")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("页面正常")
        time.sleep(2)
