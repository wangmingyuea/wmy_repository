...
author: daizhiwei
Project: 出行赠险-万元出行保障限时送（无到期时间）
interlinkage: https://t.shijiala.com/17kq
'''
import unittest
import warnings
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_chuxingzengxian(self):
        time.sleep(2)
        self.driver.find_element_by_id('com.tencent.mm:id/al_').send_keys(
        'https://t.shijiala.com/17kq')
        time.sleep(1)
        self.driver.find_element_by_id ('com.tencent.mm:id/anv').click ( )  # 点击发送按钮
        time.sleep(0.5)
        TouchAction (self.driver).tap (x=643.5, y=893.2).release ( ).perform ( )  # 点击链接
        time.sleep(3)

        # 页面返回 获取title内容

        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="百万保障免费领"]'))
        self.assertEqual(a.text, "百万保障免费领")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("页面正常")
