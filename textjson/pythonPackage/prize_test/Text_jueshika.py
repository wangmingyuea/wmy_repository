...
author: daizhiwei
Project: 天天省呗爵士卡-吃喝玩乐低至1元起（无到期时间）
interlinkage: https://client.ttshengbei.com/sale/channel/landing_page/SJZXDZK
'''
import unittest
import warnings
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_jueshika(self):
        time.sleep(2)
        self.driver.find_element_by_id('com.tencent.mm:id/al_').send_keys(
        'https://client.ttshengbei.com/sale/channel/landing_page/SJZXDZK')
        time.sleep(1)
        ...
        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="爵士卡"]'))
        self.assertEqual(a.text, "爵士卡")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("页面正常")
