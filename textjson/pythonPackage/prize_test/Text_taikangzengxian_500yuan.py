...
author: daizhiwei
Project: 泰康赠险-免费领500元重疾保障（无到期时间）
interlinkage: https://121baoxian.guanghua.com/m/activity/mall/tkzjbz.html?channel=jdbtkzjbz
'''
import unittest
import warnings
...
class TestSuite_API(unittest.TestCase):
    ...
    def test_case_link_taikangzengxian(self):
        time.sleep(2)
        self.driver.find_element_by_id('com.tencent.mm:id/al_').send_keys(
        'https://121baoxian.guanghua.com/m/activity/mall/tkzjbz.html?channel=jdbtkzjbz')
        time.sleep(1)
        ...
        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="免费领500元重疾保障"]'))
        self.assertEqual(a.text, "免费领500元重疾保障")  # 跳转奖品页面之后，对比title，看是否领取成功
        print("页面正常")
