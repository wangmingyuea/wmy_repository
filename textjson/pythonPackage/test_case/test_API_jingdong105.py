...
class TestSuite_API(unittest.TestCase):
    ...
    # 定义测试京东105元减5元券链接方法
    def test_case_link_jingdongquan(self):
        ...
        time.sleep(0.5)
        TouchAction(self.driver).tap(x=471, y=857).release().perform()
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="京东满105减5"]'))
        # self.assertEqual (a.text.encode('UTF-8'), "京东满105减5", '链接异常')
        self.assertEqual (a.text, "京东满105减5")
        # time.sleep(0.5)
