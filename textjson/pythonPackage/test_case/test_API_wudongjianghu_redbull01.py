...
class TestSuite_API(unittest.TestCase):
    ...
    # 定义测试武动江湖-redbull01域名链接方法
    def test_case_wudongjianghu_redbull01(self):
        ...
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys (
            'http://api.redbull01.cn/connector/theconnect/targetCon.shtml?type=3&id=2c9080e26bb0f6d6016bbc49c179001b')
        self.driver.find_element_by_id('com.tencent.mm:id/aql').click()
        time.sleep(0.5)
        TouchAction(self.driver).tap(x=650.4, y=1153.5).release().perform()
        time.sleep(2)
        a = WebDriverWait(self.driver, 5).until(lambda x: x.find_element_by_xpath(
            '//android.widget.TextView[@text="武动江湖"]'))
        # self.assertEqual (a.text.encode('UTF-8'), "武动江湖", '链接异常')
        self.assertEqual (a.text, "武动江湖")
        # time.sleep(0.5)
