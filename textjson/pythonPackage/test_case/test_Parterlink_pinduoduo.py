...
class TestSuite_Partenlink(unittest.TestCase):
    ...
    def test_case_pinduoduo(self):
        self.driver.find_element_by_id ('com.tencent.mm:id/aqe').send_keys ('https://p.pinduoduo.com/IEbrlkdr')
        self.driver.find_element_by_id ('com.tencent.mm:id/aql').click ( )
        time.sleep(0.5)
        TouchAction (self.driver).tap (x=480, y=853).release ( ).perform ( )
        time.sleep(0.5)
        a = WebDriverWait(self.driver, 10).until(lambda x: x.find_element_by_xpath (
            '//android.widget.TextView[@text="拼多多"]'))
        self.assertEqual(a.text, "拼多多")
