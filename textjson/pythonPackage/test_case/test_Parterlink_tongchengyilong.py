...
class TestSuite_Partenlink(unittest.TestCase):
    ...
    def test_case_tongchengyilong(self):
        self.driver.find_element_by_id('com.tencent.mm:id/aqe').send_keys(
            'https://wx.17u.cn/marketactivity/platact/f0bd659670c9ebb5da1fe1bdeab49615/?wvc3=1&refid=787109561')
        self.driver.find_element_by_id('com.tencent.mm:id/aql').click()
        time.sleep(0.5)
        TouchAction (self.driver).tap (x=600, y=800).release ( ).perform ( )
        time.sleep(0.5)
        a = WebDriverWait(self.driver, 15).until(lambda x: x.find_element_by_xpath (
            '//android.widget.TextView[@text="同程艺龙"]'))
        self.assertEqual(a.text, "同程艺龙")
